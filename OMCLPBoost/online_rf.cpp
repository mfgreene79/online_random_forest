...
//version to construct from parameters
RandomTest::RandomTest(const int& numClasses, int feature, double threshold,
		       VectorXd trueStats, VectorXd falseStats) : 
  m_feature(feature), m_threshold(threshold), m_numClasses(&numClasses),
  m_trueStats(trueStats), m_falseStats(falseStats), m_trueCount(0.0), m_falseCount(0.0) {
...
//version to create from information about the node
OnlineNode::OnlineNode(const VectorXd& nodeParms, const Hyperparameters& hp,
  ...
  if(m_isLeaf == false) { //when not a leaf create the best test
    ...
    bt = new RandomTest(numClasses, feature, threshold, trueStats, falseStats);
    m_bestTest = bt;
  }
  ...
  for(int i=0; i < m_hp->numRandomTests; ++i) {
    ...
    rt = new RandomTest(numClasses, feature, threshold, trueStats, falseStats);
...
void OnlineNode::update(const MatrixXd& treeParms) {
  if(m_isLeaf == false) { // if its a leaf then theres no splitting to do
    VectorXd thisNodeParms = exportParms();

    // get the children numbers
    int rightNodeNumber = static_cast<int>(thisNodeParms(2));
    int leftNodeNumber = static_cast<int>(thisNodeParms(3));
...
VectorXd OnlineNode::exportParms() {
  ...
  int vec_size = 13 + 3 * *m_numClasses + 2 * m_hp->numRandomTests * (1 + *m_numClasses);
...
OnlineTree::OnlineTree(const Hyperparameters& hp, const int& numClasses, 
  ...
  //initialize here - will get updated through pointers
  m_oobe = 0.0;
  m_counter = 0.0;
  m_numNodes = 0;
